...
class PlannerNode(object):
  ...
  def get_plan_cb(self, req):
    ...
    def get_one_plan(self,point1,point2):
    	self.source_lock.acquire()
    	self.source_pose = point1[:2]
    	self.source_yaw = point1[2]
  	self.source_updated = True
 	self.source_lock.release()    
    
  	self.target_lock.acquire()
  	self.target_pose = point2[:2]
  	self.target_yaw = point2[2]
    ...
    points=np.array(req.point).reshape(-1, 3)
    gpr = GetPlanResponse()
    for i in range(len(points)-1):
        gpr.plan+=get_one_plan(self,points[i],points[i+1]).tolist()
    #gpr.plan=get_one_plan(self,req.source,req.point2).tolist()+get_one_plan(self,req.point2,req.point3).tolist()+get_one_plan(self,req.point3,req.point4).tolist()
    #gpr.plan+=get_one_plan(self,req.point4,req.point5).tolist()+get_one_plan(self,req.point5,req.point6).tolist()+get_one_plan(self,req.point6,req.point7).tolist()
    gpr.success=True
    self.cur_plan=np.array(gpr.plan)
    gpr.plan=np.array(gpr.plan).flatten().tolist()
    gpr.success=True
    return gpr
...
if __name__ == '__main__':
  ...
  car_width = rospy.get_param("/car_kinematics/car_width", 0.4)
  car_length = rospy.get_param("/car_kinematics/car_length", 0.4)
