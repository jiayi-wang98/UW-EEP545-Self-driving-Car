...
class ParticleFilter():
  ...
  def initialize_global(self):
    ...
    permissible_idx=list()
    for i in range(self.map_info.height):
      for j in range(self.map_info.width):
        if (self.permissible_region[i][j]==1):
          permissible_idx.append([j,i])
    
    # Uniformally sample from in-bounds regions
    idx=np.random.choice(len(permissible_idx),self.N_PARTICLES)
    permissible_idx=np.array(permissible_idx)
    self.particles[:,:2]=permissible_idx[idx][:,:]
    self.particles[:,2]=np.random.rand(self.particles.shape[0])*2*np.pi-np.pi
  ...
  def expected_pose(self):
    # YOUR CODE HERE
    pose_particle=np.zeros(3)
    pose_particle[0]=float(np.sum(self.particles[:,0]))/self.N_PARTICLES
    pose_particle[1]=float(np.sum(self.particles[:,1]))/self.N_PARTICLES
    pose_particle[2]=np.arctan2(np.sum(np.sin(self.particles[:,2])),np.sum(np.cos(self.particles[:,2])))
    return pose_particle
  ...
  def clicked_pose_cb(self, msg):
    ...
    # YOUR CODE HERE
    gaussian_x,gaussian_y=np.random.multivariate_normal([msg.pose.pose.position.x,msg.pose.pose.position.y],[[0.5,0],[0,0.5]],self.N_PARTICLES).T
    self.particles[:,0]=gaussian_x
    self.particles[:,1]=gaussian_y
    self.particles[:,2]=np.random.normal(Utils.quaternion_to_angle(msg.pose.pose.orientation),0.05,self.N_PARTICLES)
    self.weights = np.ones(self.N_PARTICLES) / float(self.N_PARTICLES)
...
if __name__ == '__main__':
  ...
  speed_to_erpm_gain = float(rospy.get_param("/car/vesc/speed_to_erpm_gain", 4000))   # Gain conversion param from rpm to speed
  steering_angle_to_servo_offset = float(rospy.get_param("/car/vesc/steering_angle_to_servo_offset", 0.45)) # Offset conversion param from servo position to steering angle
  steering_angle_to_servo_gain = float(rospy.get_param("/car/vesc/steering_angle_to_servo_gain", -0.95)) # Gain conversion param from servo position to steering angle    
