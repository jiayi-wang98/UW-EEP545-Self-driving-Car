...
Z_SHORT = 0.3 # Weight for short reading
lamd_short=0.001
...
class SensorModel:
  ...
  def __init__(self, scan_topic, laser_ray_step, exclude_max_range_rays, 
    ...
    self.idx = None #The index of the downsampled ranges
  ...
  def lidar_cb(self, msg):
    ...
    if not isinstance(self.idx, np.ndarray):  
      #caching downsampled ranges index
      self.idx=np.arange(0, len(msg.ranges), self.LASER_RAY_STEP,dtype=np.int16)
      #caching downsampled_angles
      self.downsampled_angles=np.arange(msg.angle_min,msg.angle_max,self.LASER_RAY_STEP*msg.angle_increment,dtype=np.float32)
    
    #create obs
    obs=(np.array(msg.ranges,dtype=np.float32)[self.idx],self.downsampled_angles)

    #translate Nan or 0.0 to self.MAX_RANGE_METERS
    obs[0][(obs[0]==0.0) + (np.isnan(obs[0]))]=self.MAX_RANGE_METERS
  ...
  def precompute_sensor_model(self, max_range_px):
    ...
    for d in xrange(table_width):
      ...
      #p_short
      p_short=np.zeros(table_width)
      if d==0:
        p_short[0]=1
      else:
       yida=1/(1-np.exp(-lamd_short*d))
       for i in xrange(table_width):
         if i<=d:
           p_short[i]=yida*lamd_short*np.exp(-lamd_short*i)

      #p_hit
      p_hit=np.zeros(table_width)     
      for i in xrange(table_width):
        p_hit[i]=1/(np.sqrt(2*np.pi)*SIGMA_HIT)*np.exp(-0.5*(i-d)**2/SIGMA_HIT**2)
      p_hit=p_hit/sum(p_hit)

      for r in xrange(table_width):
        sensor_model_table[r][d]=Z_SHORT * p_short[r] +Z_MAX * (r==max_range_px)+Z_RAND * (r<=max_range_px)/max_range_px+Z_HIT * p_hit[r] 
    #print(sensor_model_table)
    #plt.plot(range(table_width),sensor_model_table[:,100])
    #plt.show()
